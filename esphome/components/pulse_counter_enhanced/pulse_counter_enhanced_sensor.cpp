#include "pulse_counter_enhanced_sensor.h"
#include "esphome/core/log.h"

namespace esphome {
namespace pulse_counter_enhanced {

static const char *const TAG = "pulse_counter_enhanced";

const char *const EDGE_MODE_TO_STRING[] = {"DISABLE", "INCREMENT", "DECREMENT"};

#ifdef HAS_PCNT
PulseCounterEnhancedStorageBase *get_storage(bool hw_pcnt) {
  return (hw_pcnt ? (PulseCounterEnhancedStorageBase *) (new HwPulseCounterEnhancedStorage)
                  : (PulseCounterEnhancedStorageBase *) (new BasicPulseCounterEnhancedStorage));
}
#else   // HAS_PCNT
PulseCounterEnhancedStorageBase *get_storage(bool) { return new BasicPulseCounterEnhancedStorage; }
#endif  // HAS_PCNT

void IRAM_ATTR BasicPulseCounterEnhancedStorage::gpio_intr(BasicPulseCounterEnhancedStorage *arg) {
  ...
  PulseCounterEnhancedCountMode mode = arg->isr_pin.digital_read() ? arg->rising_edge_mode : arg->falling_edge_mode;
  switch (mode) {
...
void BasicPulseCounterEnhancedStorage::reset() {
  this->counter = 0;
  this->last_value = 0;
  this->last_pulse = 0;
}

bool BasicPulseCounterEnhancedStorage::pulse_counter_setup(InternalGPIOPin *pin) {
  this->pin = pin;
  this->pin->setup();
  this->isr_pin = this->pin->to_isr();
  this->pin->attach_interrupt(BasicPulseCounterEnhancedStorage::gpio_intr, this, gpio::INTERRUPT_ANY_EDGE);
  return true;
}

pulse_counter_t BasicPulseCounterEnhancedStorage::read_raw_value() {
...
#ifdef HAS_PCNT
void HwPulseCounterEnhancedStorage::reset() {
  this->last_value = 0;
  pcnt_counter_clear(this->pcnt_unit);
}

bool HwPulseCounterEnhancedStorage::pulse_counter_setup(InternalGPIOPin *pin) {
...
pulse_counter_t HwPulseCounterEnhancedStorage::read_raw_value() {
...
void PulseCounterEnhancedSensor::setup() {
  ESP_LOGCONFIG(TAG, "Setting up pulse counter enhanced '%s'...", this->name_.c_str());
...
void PulseCounterEnhancedSensor::set_total_pulses(uint32_t pulses) {
  this->current_total_ = pulses;
  this->total_sensor_->publish_state(pulses);
}

void PulseCounterEnhancedSensor::dump_config() {
...
void PulseCounterEnhancedSensor::update() {
